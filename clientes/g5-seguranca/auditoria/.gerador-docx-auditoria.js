...
const SRC='/home/user/Markseg-geral-clientes/clientes/g5-seguranca/auditoria/Auditoria-Site-G5-Seguranca.md';
const OUT='/home/user/Markseg-geral-clientes/clientes/g5-seguranca/auditoria/G5-Seguranca-Auditoria-Site.docx';
...
kids.push(new Paragraph({children:[new TextRun({text:'Auditoria Completa do Site',size:36,color:GRAFITE})],spacing:{after:60},alignment:AlignmentType.CENTER}));
kids.push(new Paragraph({children:[new TextRun({text:'SEO, estrutura, design, conversão e palavras-chave | Setembro de 2026',size:22,color:'666666'})],spacing:{after:600},alignment:AlignmentType.CENTER}));
kids.push(new Paragraph({border:{bottom:{style:BorderStyle.SINGLE,size:6,color:AZUL}},spacing:{after:400}}));
kids.push(new Paragraph({children:[new TextRun({text:'O que este relatório cobre',bold:true,size:24,color:AZUL})],spacing:{after:160},alignment:AlignmentType.CENTER}));
[['Coleta','148 URLs rastreadas no site em 18/09/2026','Sitemaps oficiais, cabeçalhos HTTP e metadados'],
 ['Desempenho','Medição em navegador real, viewport de celular','TTFB, FCP, LCP, CLS e peso transferido'],
 ['Conteúdo','116 posts e 23 páginas analisados','Extensão, duplicidade, estrutura e links internos'],
 ['Palavras-chave','Cruzamento com o estudo de agosto de 2026','Cobertura, lacunas e canibalização'],
 ['Plano de ação','30 itens priorizados em três horizontes','30, 60 e 90 dias']].forEach(([d,t,s2])=>{
  ...
  kids.push(new Paragraph({children:[new TextRun({text:s2,size:18,italics:true,color:'808080'})],spacing:{after:140},alignment:AlignmentType.CENTER}));
});
...
while(i<lines.length){
  ...
  if(t.startsWith('### ')){
    kids.push(new Paragraph({heading:HeadingLevel.HEADING_3,children:[new TextRun({text:t.slice(4).trim(),bold:true,size:23,color:GRAFITE})],spacing:{before:240,after:120}}));
    i++;continue;
  }
  if(t.startsWith('## ')){
    const txt=t.slice(3).trim();
    if(/^\d+\./.test(txt)) kids.push(new Paragraph({children:[new PageBreak()]}));
    kids.push(new Paragraph({heading:HeadingLevel.HEADING_1,children:[new TextRun({text:txt,bold:true,size:28,color:AZUL})],spacing:{before:200,after:160},border:{bottom:{style:BorderStyle.SINGLE,size:4,color:'D9D9D9'}}}));
    i++;continue;
  }
...
const doc=new Document({
  ...
  sections:[{properties:{page:{margin:{top:convertInchesToTwip(1),bottom:convertInchesToTwip(1),left:convertInchesToTwip(1),right:convertInchesToTwip(1)}}},
    headers:{default:new Header({children:[new Paragraph({alignment:AlignmentType.RIGHT,children:[new TextRun({text:'G5 Segurança | Auditoria do site | Setembro de 2026',size:16,color:'999999'})]})]})},
